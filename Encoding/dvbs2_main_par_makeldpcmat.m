snr=10;%Energy per symbol by No
numF=10;
needed_coderate=0.9;5/6;1/2;
subsystemType='16QM'; %QPSK 16QM 64QM 256Q
existingDBVs=[] ;
...
b4Enc_bitSET=zeros(numFrames,dvb.BCHMessageLength);
...
for frameCnt=1:numFrames
    ...
    bbFrameTx(1:dvb.BCHMessageLength) = ...
          logical(randi([0 1], dvb.BCHMessageLength, 1));
    
    b4Enc_bitSET(frameCnt,:)=bbFrameTx(1:dvb.BCHMessageLength);
...
%     ldpcEncOut=[ldpcEncOut; 0; 0] 
    ...
    % Error statistics
    comparedBits = xor(bbFrameRx, bbFrameTx(1:dvb.BCHMessageLength));
